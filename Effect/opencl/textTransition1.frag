...
vec4 FUNCNAME(vec2 tc)
{
	...
	if(inORout == 0)
	{
		...
		progress = ( 3.0*progress*progress - 2.0 * progress*progress*progress - 1.0)*(visWidth);//3*x*x - 2*x*x*x -1
	}else
	{
		bgCol = INPUT2(  vec2(tempTc.x, 1.0 - tempTc.y) );
		progress = ( 2.0 * progress*progress*progress - 3.0*progress*progress )*(visWidth);
		//2*x*x*x- 3*x*x 
	}
	//moveX(progress);
